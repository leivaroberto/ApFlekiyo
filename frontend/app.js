...
function habilitarEdicionTelefono(clienteId, telefonoActual) {
    ...
    contenedorEdicion.innerHTML = `
        <div style="display: flex; gap: 8px; align-items: center; background: #fff; padding: 8px; border: 1px solid #ccc; border-radius: 5px; max-width: 300px;">
            <input type="text" id="nuevo-tel-${clienteId}" value="${telefonoActual === 'Sin registrar' ? '' : telefonoActual}" placeholder="Nuevo teléfono" style="padding: 5px; border: 1px solid #bdc3c7; border-radius: 4px; flex: 1;">
            <button onclick="guardarNuevoTelefono('${clienteId}')" style="background: #27ae60; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer; font-weight: bold;">Guardar</button>
        </div>
    `;
}
