...
function configurarTogglePassword() {
    document.querySelectorAll('.password-toggle').forEach((boton) => {
        ...
        if (!input || boton.dataset.bound === 'true') return;

        boton.dataset.bound = 'true';
        boton.addEventListener('click', () => {
            const mostrando = input.type === 'text';
            input.type = mostrando ? 'password' : 'text';
            boton.textContent = mostrando ? 'Mostrar' : 'Ocultar';
            boton.setAttribute('aria-label', mostrando ? 'Mostrar contraseña' : 'Ocultar contraseña');
            boton.classList.toggle('active', !mostrando);
        });
    });
}
...
function renderizarTurnos(turnos) {
    ...
    turnos.forEach(turno => {
        ...
        if (esFinalizado) {
            ...
            controlesHTML = `<span style="color: #27ae60; font-weight: bold; font-size: 14px;"><i data-lucide="circle-check" class="app-icon" aria-hidden="true"></i> Servicio Finalizado</span>`;
            ...
            `;
            botonBorrarHTML = `<button onclick="borrarTurno('${turno.id}')" style="background: transparent; border: none; font-size: 18px; cursor: pointer;" title="Borrar Turno" aria-label="Borrar turno"><i data-lucide="x" class="app-icon" aria-hidden="true"></i></button>`;
        }
        
        div.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: start; width: 100%;">
                <div>
                    <strong style="font-size: 16px; color: #2c3e50;"><i data-lucide="clock" class="app-icon" aria-hidden="true"></i> ${horaFormateada} | <i data-lucide="user-round" class="app-icon" aria-hidden="true"></i> ${nombreCliente} ${apellidoCliente}</strong><br>
                    <span style="color: #e67e22; font-size: 14px; font-weight: 500; display: inline-block; margin-top: 4px;"><i data-lucide="clipboard-list" class="app-icon" aria-hidden="true"></i> ${trabajo}</span><br>
...
async function buscarCliente() {
    ...
    // 1. Buscamos los datos básicos del cliente incluyendo peluqueria_id
    ...
    for (const cliente of clientes) {
        ...
        html += `<div style="background: #f9f9f9; padding: 15px; margin-bottom: 15px; border-radius: 8px; border: 1px solid #ddd;">
            <h4 style="margin-top:0; color:#2c3e50; font-size:18px;"><i data-lucide="user-round" class="app-icon" aria-hidden="true"></i> ${cliente.nombre} ${cliente.apellido || ''}</h4>
                
                <!-- Sección de Teléfono con opción de edición -->
                <p style="margin: 5px 0; display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                    <strong>Teléfono:</strong> 
                    <span id="texto-tel-${cliente.id}">${cliente.telefono || 'Sin registrar'}</span>
                    
                    <button onclick="habilitarEdicionTelefono('${cliente.id}', '${cliente.telefono || ''}')" style="background: #3498db; color: white; border: none; padding: 3px 8px; border-radius: 4px; cursor: pointer; font-size: 12px;">
                        <i data-lucide="pencil" class="app-icon" aria-hidden="true"></i> Modificar
                    </button>
                </p>
                <div id="contenedor-editar-tel-${cliente.id}" style="margin-top: 5px; display: none;"></div>
                
                <h5 style="margin-bottom: 5px; margin-top: 15px; color:#d35400;"><i data-lucide="calendar-days" class="app-icon" aria-hidden="true"></i> Historial de Atención y Trabajos:</h5>`;
        ...
        if (!turnos || turnos.length === 0) {
            ...
            for (const turno of turnos) {
                ...
                const estado = turno.estado === 'finalizado'
                    ? '<i data-lucide="circle-check" class="app-icon" aria-hidden="true"></i> Finalizado'
                    : `<i data-lucide="clock" class="app-icon" aria-hidden="true"></i> ${turno.estado}`;
                
                html += `<li style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px dashed #ccc;">
                            <strong style="color: #2c3e50;"><i data-lucide="calendar-days" class="app-icon" aria-hidden="true"></i> ${fecha} a las ${hora}hs</strong> | ${estado}<br>
                            <span style="color: #2980b9;"><i data-lucide="scissors" class="app-icon" aria-hidden="true"></i> Trabajo: <strong>${trabajoRealizado}</strong></span><br>
...
// Habilita el input de edición rápida para el teléfono del cliente
function habilitarEdicionTelefono(clienteId, telefonoActual) {
    const contenedorEdicion = document.getElementById(`contenedor-editar-tel-${clienteId}`);
    
    if (contenedorEdicion.style.display === 'block') {
    ...
    contenedorEdicion.innerHTML = `
        <div style="display: flex; gap: 8px; align-items: center; background: #fff; padding: 8px; border: 1px solid #ccc; border-radius: 5px; width: 100%; max-width: 300px; box-sizing: border-box;">
            <input type="text" id="nuevo-tel-${clienteId}" value="${telefonoActual === 'Sin registrar' ? '' : telefonoActual}" placeholder="Nuevo teléfono" style="padding: 5px; border: 1px solid #bdc3c7; border-radius: 4px; flex: 1 1 auto; min-width: 0; box-sizing: border-box;">
            <button onclick="guardarNuevoTelefono('${clienteId}')" style="background: #27ae60; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer; font-weight: bold; flex-shrink: 0; white-space: nowrap;">Guardar</button>
        </div>
    `;
}

// Guarda el número actualizado en Supabase
async function guardarNuevoTelefono(clienteId) {
    const nuevoTelefono = document.getElementById(`nuevo-tel-${clienteId}`).value.trim();

    if (!nuevoTelefono) {
        return alert("El número de teléfono no puede estar vacío.");
    }
    ...
    if (error) {
        ...
        alert("Hubo un error al actualizar el teléfono en la base de datos.");
    } else {
        alert("¡Teléfono actualizado con éxito!");
        // Refrescamos la búsqueda automáticamente para ver el cambio reflejado
        buscarCliente();
    }
}
...
async function cargarCajaMensual() {
    ...
    for (const [nombre, monto] of Object.entries(liquidacion)) {
        htmlComisiones += `<div class="comision-mes-item"><strong><i data-lucide="user-round" class="app-icon" aria-hidden="true"></i> ${nombre}</strong><span style="color:#e67e22; font-weight:bold;">$${monto.toLocaleString('es-AR')}</span></div>`;
    }
...
// Función para guardar un nuevo producto en el pañol
async function crearProductoNuevo() {
    ...
    if (error) {
        ...
        if (mensaje) {
            mensaje.style.color = '#27ae60';
            mensaje.innerText = "¡Producto guardado exitosamente!";
            setTimeout(() => mensaje.innerText = '', 3000);
        }
...
async function cargarProductosAdmin() {
    ...
    if (insumos.length === 0) {
        contenedor.innerHTML = '<p>No hay productos registrados en el pañol.</p>';
        if (panelTotal) panelTotal.innerText = '0';
        if (panelAlertas) panelAlertas.innerText = '0';
        return;
    }
            ...
            <tr style="background-color: ${fondoFila}; border-bottom: 1px solid #eee;">
                ...
                <td style="padding: 15px; text-align: center;">
                    <div style="display: inline-flex; align-items: center; gap: 8px; background: #ecf0f1; padding: 6px; border-radius: 6px;">
                        ...
                        <button onclick="modificarStock(${insumo.id}, ${insumo.stock_gramos}, '${unidad}', 'sumar')" style="background: #27ae60; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-weight: bold;" title="Agregar al stock" aria-label="Agregar al stock"><i data-lucide="plus" class="app-icon" aria-hidden="true"></i></button>
                        <button onclick="modificarStock(${insumo.id}, ${insumo.stock_gramos}, '${unidad}', 'restar')" style="background: #e74c3c; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-weight: bold;" title="Descontar del stock" aria-label="Descontar del stock"><i data-lucide="minus" class="app-icon" aria-hidden="true"></i></button>
                        <button onclick="eliminarProducto(${insumo.id})" style="background: #7f1d1d; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer; font-weight: bold;" title="Eliminar producto" aria-label="Eliminar ${insumo.nombre}"><i data-lucide="x" class="app-icon" aria-hidden="true"></i></button>
                    </div>
                </td>
            </tr>
        `;
...
async function eliminarProducto(insumoId) {
    if (!haySesionActiva()) return;
    if (!confirm("¿Estás seguro que deseas eliminar este producto del inventario?")) return;
    ...
    turnos.forEach(turno => {
        ...
        if (cliente && cliente.telefono && cliente.telefono !== 'Sin asignar') {
            ...
            if (numeroLimpio.length > 5) { 
                ...
                textoBtn = '<i data-lucide="message-circle" class="app-icon" aria-hidden="true"></i> Enviar WhatsApp';
                estiloBtn = ''; 
            }
        }
        ...
        html += `
            <div class="turno-proximo-card">
                <div class="turno-proximo-info">
                    <strong style="text-transform: capitalize; color:#2c3e50; font-size:16px;"><i data-lucide="calendar-days" class="app-icon" aria-hidden="true"></i> ${fechaFormateada}</strong>
                    <span><i data-lucide="user-round" class="app-icon" aria-hidden="true"></i> Cliente: ${cliente ? cliente.nombre + ' ' + (cliente.apellido || '') : 'Desconocido'}</span>
                    <span><i data-lucide="scissors" class="app-icon" aria-hidden="true"></i> Profesional: ${peluquero}</span>
                    <span style="color:#7f8c8d; font-size:13px;"><i data-lucide="map-pin" class="app-icon" aria-hidden="true"></i> Estado actual: ${turno.estado}</span>
                </div>
...
if (window.lucide) {
    ...
    const iconObserver = new MutationObserver(mutations => {
        const hayIconosNuevos = mutations.some(({ addedNodes }) =>
        ...
        if (hayIconosNuevos) window.lucide.createIcons();
    });
